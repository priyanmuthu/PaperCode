...
class CodeFile:
    ...
    def generate_syntax_tree(self):
        ...
        self.syntax_tree = structure_visitor.syntax_tree
    ...
    def generate_partitions(self):
        ...
        # Include everython before the first non-Node
            ...
            partitions.append(self.get_partition(1, len(self.all_lines)+1, Node.Node))
        else:
            ...
            partitions.append(self.get_partition(start_line, end_line-1, Node.Node))
            ...
            for tnode in tail_nodes:
                ...
                if type(tnode) is Node.ClassNode:
                    partitions.append(self.get_partition(tnode.start_pos.line, tnode.body_start_pos.line - 1, Node.ClassNode))
                elif type(tnode) is Node.FunctionNode:
                    partitions.append(self.get_partition(tnode.start_pos.line, tnode.end_pos.line, Node.FunctionNode))
    ...
    def get_partition_from_node(self, cst_node):
        if type(cst_node) is Node.Node:
            if len(cst_node.children) == 0:
                return self.get_partition(1, len(self.all_lines)+1, Node.Node)
            return self.get_partition(1, cst_node.children[0].start_pos.line-1, Node.Node)
        elif type(cst_node) is Node.ClassNode:
            return self.get_partition(cst_node.start_pos.line, cst_node.body_start_pos.line - 1, Node.ClassNode)
        elif type(cst_node) is Node.FunctionNode:
            return self.get_partition(cst_node.start_pos.line, cst_node.end_pos.line, Node.FunctionNode)