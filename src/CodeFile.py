...
class CodeFile:
    ...
    def generate_partitions(self, paper_options = print_paper['A4']):
        ...
        # Include everython before the first non-Node
            ...
            partitions.append(self.get_partition(1, len(self.all_lines)+1))
        else:
            ...
            partitions.append(self.get_partition(start_line, end_line-1))
            ...
            for tnode in tail_nodes:
                ...
                if type(tnode) is Node.ClassNode:
                    partitions.append(self.get_partition(tnode.start_pos.line, tnode.body_start_pos.line - 1))
                elif type(tnode) is Node.FunctionNode:
                    partitions.append(self.get_partition(tnode.start_pos.line, tnode.end_pos.line))
            
        return partitions

    def get_partition(self, start_line, end_line):
